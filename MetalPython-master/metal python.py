...
def instructions():
    '''instructions screen'''
    #Entities
    bkgd=pygame.image.load('MetalPython-master\\images\\instructions.png').convert()   
...
def title_screen():
    '''title screen'''
    #Entities
    bkgd=pygame.image.load(f'MetalPython-master\\images\\title screen.jpg').convert()    
    ...
    #sound
    sfx=pygame.mixer.Sound(f'MetalPython-master\\sounds\\menu.wav')
    sfx.play()
...
def level1():
    ...
    #     background
    clean_bkgd=pygame.image.load('MetalPython-master\\images\\bkgd.png').convert()
    ...
    #     sound
    pygame.mixer.music.load('MetalPython-master\\sounds\\music.mp3')
    ...
    while keepGoing:
        ...
        for bullet, enemies in pygame.sprite.groupcollide(pBulletsGrp, enemiesGrp, False, False).items():
            # Check if there are any enemies and if the first enemy is not dying
            if enemies and not enemies[0].get_dying():
                # Remove the bullet
                bullet.kill()
                # Trigger the enemy's death
                enemies[0].die()

        # Grenade collision with enemies
        for grenade, enemies in pygame.sprite.groupcollide(grenadeGrp, enemiesGrp, False, True).items():
            if enemies:
                grenade.explode()
        ...
        for grenade, platforms in pygame.sprite.groupcollide(grenadeGrp, platforms, False, False).items():
            if platforms:
                grenade.explode()
...
def gameover():
    ...
    #sound
    pygame.mixer.music.load('MetalPython-master\\sounds\\gameover.wav')
...
def boss(prev_player,prev_tank=None):
    ...
    #     background
    clean_bkgd=pygame.image.load('MetalPython-master\\images\\bossbkgd2.png').convert()
    ...
    #     sound
    pygame.mixer.music.load('MetalPython-master\\sounds\\boss.mp3')
    pygame.mixer.music.play(-1)  
    missioncomplete=pygame.mixer.Sound('MetalPython-master\\sounds\\mission complete.wav')
    ...
    while keepGoing:
        ...
        if boss.get_dead():
            ...
            screen.blit(pygame.image.load('MetalPython-master\\images\\mission complete.png').convert_alpha(),(109,167))
            pygame.display.flip() 
