...
def load_all_gfx(directory, colorkey=(255,0,255), accept=('.png', 'jpg', 'bmp')):
    ...
    for pic in os.listdir(graphx_directory):
        ...
        if ext.lower() in accept:
            img = pg.image.load(os.path.join(graphx_directory, pic))
            if img.get_alpha():
...
def load_all_music(directory, accept=('.wav', '.mp3', '.ogg', '.mdi')):
    ...
    for song in os.listdir(music_directory):
        ...
        if ext.lower() in accept:
            songs[name] = os.path.join(music_directory, song)
    return songs
...
def load_all_sfx(directory, accept=('.wav','.mpe','.ogg','.mdi')):
    ...
    for fx in os.listdir(sound_directory):
        ...
        if ext.lower() in accept:
            effects[name] = pg.mixer.Sound(os.path.join(sound_directory, fx))
    return effects
